...
class GRUAutoencoderModel(SequenceModel):
    def __init__(self, sequence_length=7, n_features=10, encoding_dim=5, learning_rate=0.001, epochs=50, batch_size=32):
        """
        GRU Autoencoder for temporal anomaly detection
        Faster convergence than LSTM, less prone to overfitting
        ...
        """
        super().__init__(model_name="gru_autoencoder", sequence_length=sequence_length, n_features=n_features)
    ...
    def build_model(self):
        """Build GRU Autoencoder architecture"""
        # Encoder
        inputs = Input(shape=(self.sequence_length, self.n_features))
        encoded = GRU(self.encoding_dim, activation='relu')(inputs)
        
        # Decoder
        decoded = RepeatVector(self.sequence_length)(encoded)
        decoded = GRU(self.n_features, activation='relu', return_sequences=True)(decoded)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray = None, validation_split: float = 0.2, verbose: int = 1) -> 'GRUAutoencoderModel':
        """
    ...
    def load_model(self, filepath: str) -> 'GRUAutoencoderModel':
        """
        Load trained model
        ...
        encoded_layer = self.model.get_layer(index=1).output  # Assuming GRU layer is at index 1
        self.encoder = Model(inputs=inputs, outputs=encoded_layer)
        self.is_trained = True
        return self

def evaluate_gru_ae(gru_ae, X_test, y_true_binary=None):
    """
    Evaluate GRU Autoencoder performance
    
    Args:
        gru_ae: Trained GRUAutoencoder
        X_test: Test sequences
        y_true_binary: True binary labels (optional)
    """
    # Get reconstruction errors and scores
    errors = gru_ae.reconstruction_errors(X_test)
    scores = gru_ae.anomaly_scores(X_test)
